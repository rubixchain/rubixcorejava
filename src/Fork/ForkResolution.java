...
//        ForkResolutionLogger.debug("Token Details:" + tokenDetails);
        ...
        String currentSender = tokenDetails.get("currentSender").toString();
        String currentReceiver = tokenDetails.get("currentReceiver").toString();
        ...
        ForkResolutionLogger.debug("Providers for " + token + " is "+pinOwnersArrayTransferToken.toString());

        //TODO remove current sender and receiver from the list
        JSONArray removerCurrentProvidersArray = new JSONArray();
        for(int i =0; i < pinOwnersArrayTransferToken.size(); i++) {
        	if(!(pinOwnersArrayTransferToken.get(i).toString().equals(currentReceiver) || pinOwnersArrayTransferToken.get(i).toString().equals(currentSender)))
        		removerCurrentProvidersArray.put(pinOwnersArrayTransferToken.get(i).toString());
        }
           
        ForkResolutionLogger.debug("Providers for " + token + " after cleanup is "+removerCurrentProvidersArray.toString());

        
        pinOwnersArrayTransferToken = new ArrayList();
        for(int i=0; i<removerCurrentProvidersArray.length(); i++)
        	pinOwnersArrayTransferToken.add(removerCurrentProvidersArray.get(i).toString());
        
        if (pinOwnersArrayTransferToken.size() > 0) {
...
//            ForkResolutionLogger.debug("Last Object of token " + token + ": " + lastObject);
    ...
    public static boolean verifyUnpledgedToken(String transferToken, String pledgedToken, String currentSender, String currentReceiver) throws IOException, InterruptedException, JSONException {
        ...
        JSONArray removerCurrentProvidersArray = new JSONArray();
        for(int i =0; i < pinListTransferToken.size(); i++) {
        	if(!(pinListTransferToken.get(i).toString().equals(currentReceiver) || pinListTransferToken.get(i).toString().equals(currentSender)))
        		removerCurrentProvidersArray.put(pinListTransferToken.get(i).toString());
        }
           
        pinListTransferToken = new ArrayList();
        for(int i=0; i<removerCurrentProvidersArray.length(); i++)
        	pinListTransferToken.add(removerCurrentProvidersArray.get(i).toString());
