...
public class DataCommitter {
    ...
    public static JSONObject Commit(String data, IPFS ipfs, int port) throws Exception {
        ...
        //DataCommitterLogger.debug("Transscation type is "+ detailsObject.getString(TRANS_TYPE));        
         ...
         */
	        	...
	        	if(!datumFolder.exists()) {
	        		DataCommitterLogger.debug("datumFolder is missing");
	        		datumFolder.mkdir();
	        		DataCommitterLogger.debug("datumFolder created");
	        ...
	        //    		getValues(datumFolderPath.concat("datumCommitChain.json"), "blockHash", "blockHash", blockHash));
	            ...
	            if(datArray.length()<1) {
	            	...
	            	/*{
		            	...
		            	DataCommitterLogger.debug("datArray.get(0).toString() "+ datArray.get(0).toString());
		            	DataCommitterLogger.debug("datArray.get(0).toString().contains(bankArray.toString()) is "+ datArray.get(0).toString().contains(bankArray.toString()));

		            	for(int i=0;i<datArray.length();i++) {
		            		for(int j=0;j<bankArray.length();j++) {
		            			if(!bankArray.get(j).toString().contains(datArray.getJSONObject(i).getString("tokenHash"))) {
			            			DataCommitterLogger.debug("Data token is "+datArray.get(0).toString()+" is no more available, updating DAT01");
		       ...
		       //     DataCommitterLogger.debug("datArray size after cleanup is "+ datArray.length());
	            ...
	            DataCommitterLogger.debug(getValues(datumFolderPath.concat("datumCommitHistory.json"), "blockHash", "blockHash", blockHash));
	          ...
	          //  JSONArray allTokens = new JSONArray();
	           ...
	           // JSONArray wholeTokenChainHash = new JSONArray();
	            ...
	            for (int i = 0; i < wholeTokensListForData.length(); i++) {
	                File token = new File(TOKENS_PATH + wholeTokensListForData.get(i));
	                File tokenchain = new File(TOKENCHAIN_PATH + wholeTokensListForData.get(i) + ".json");
	                if (!(token.exists() && tokenchain.exists())) {
	                	DataCommitterLogger.info("Tokens Not Verified : "+ wholeTokensListForData.get(i));
	                    senderMutex = false;
	                    APIResponse.put("did", senderDidIpfsHash);
	                    APIResponse.put("tid", "null");
	                    APIResponse.put("status", "Failed");
	              ...
	              //  DataCommitterLogger.debug("tokenChainFile content is "+ tokenChainFileContent);
	                JSONArray tokenChainFileArray = new JSONArray(tokenChainFileContent);
	              //  DataCommitterLogger.debug("tokenChainFileArray is "+tokenChainFileArray.toString());
	             ...
	             //   DataCommitterLogger.debug("tokenChainFileArray "+tokenChainFileArray.toString());
	       ...
	       //     DataCommitterLogger.debug("Whole token length is "+ wholeTokensListForData.length()+" and Whole token content is "+wholeTokensListForData.toString());
	            
	            
	            JSONArray positionsArray = new JSONArray();
	            String tokens=new String();
	            DataCommitterLogger.debug("All token lenght is "+dataCommitToken.length()+" Whole token length is "+ wholeTokensListForData.length());
