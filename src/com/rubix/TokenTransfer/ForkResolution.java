...
//            ForkResolutionLogger.debug("Last Object of token " + token + ": " + lastObject);
            if (lastObject.has("tokensPledgedWith")) {
                if (lastObject.getJSONArray("tokensPledgedWith") != null) {
                    ...
                    ForkResolutionLogger.debug("pledgedTokens: " + pledgeTokens);
                    for (int i = 0; i < pledgeTokens.length(); i++) {
                        ForkResolutionLogger.debug("Checking owners for pledgeToken" + pledgeTokens.get(i).toString() + " Please wait...");
                        pinOwnersArrayPledgedToken = IPFSNetwork.dhtOwnerCheck(pledgeTokens.get(i).toString());
                        if (pinOwnersArrayPledgedToken.size() != 1) {
                            resolution = false;
                            resolutionMessage = "Pledge token has more than one owner";
                        } else {
                            ForkResolutionLogger.debug("Pledged Token " + pledgeTokens.get(i).toString() + " has not multiple pins. Passed !!!");
                            return true;
                        }
                    }
                }
