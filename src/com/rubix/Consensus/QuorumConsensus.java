			...
			try {
				...
				if (operation.equals("new-credits-mining")) {
						...
						if (bankArray.length() > 0) {
							...
							if (tokenAvailableToStake) {
								...
								BufferedImage privateShare = ImageIO
										.read(new File(DATA_PATH.concat(didHash).concat("/PrivateShare.png")));
								...
								if (response.equals("alpha-stake-token-verified")) {
									...
									stakingSigns.put(STAKED_TOKEN_SIGN, getSignFromShares(
											DATA_PATH + didHash + "/PrivateShare.png", stakedTokenHash));
									// tid and sign from staker
									stakingSigns.put(MINE_TID, genesisBlock.getString("tid"));
									stakingSigns.put(MINING_TID_SIGN, getSignFromShares(
											DATA_PATH + didHash + "/PrivateShare.png", genesisBlock.getString("tid")));
									// mined token and sign from staker
									stakingSigns.put(MINED_RBT, genesisBlock.getString("tokenHash"));
									stakingSigns.put(MINED_RBT_SIGN,
									...
									FileWriter shareWriter = new FileWriter(new File(LOGGER_PATH + "stake.txt"), true);
									...
									QuorumConsensusLogger.debug("Token Staked Successfully. MINE ID: " + mineID);

									stakedTokenChainArray.put(stakingSigns);
									writeToFile(TOKENCHAIN_PATH + stakedTokenHash + ".json",
											stakedTokenChainArray.toString(), false);
					...
					if (nftDetails != null) {
						...
						/*
						 * JSONObject data1 = new JSONObject(); data1.put("sellerDID",
						...
						if (getRecData != null) {
							if (getRecData.contains("ping check")) {
								...
								if(getRecData.contains("blockHash")) {
	                        		...
	                        		QuorumConsensusLogger.debug("PID from DID "+  Dependency.getPIDfromDID(senderDidIpfsHash, Dependency.dataTableHashMap()));
	                                		//
									...
									if (Authenticate.verifySignature(detailsToVerify.toString())) {
										...
										String QuorumSignature = getSignFromShares(
												DATA_PATH + didHash + "/PrivateShare.png", quorumHash);
