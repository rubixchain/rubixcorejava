...
public class BlockCommitInitiator {

    public static Logger BlockCommitInitiatorLogger = Logger.getLogger(BlockCommitInitiator.class);
    ...
    private static synchronized boolean voteNCount(int i, int quorumSize) {
        ...
        synchronized (countLock) {
            if (quorumResponse[i] < minQuorum(quorumSize)) {
                quorumResponse[i]++;
                BlockCommitInitiatorLogger.debug("quorum response added index " + i + "  is " + quorumResponse[i]
                        + " quorumsize " + minQuorum(quorumSize));
                status = true;
            } else {
                status = false;
                BlockCommitInitiatorLogger.debug("Consensus Reached for index " + i);
            }
        }
        return status;
    }
        ...
        synchronized (signLock) {
            try {
                if (quorumSignature.length() < (minQuorum(alphaSize) + 2 * minQuorum(7))
                    ...
                    BlockCommitInitiatorLogger.debug("quorum already reached consensus " + quorumSignature.length());
                }
            } catch (JSONException e) {
                BlockCommitInitiatorLogger.error("JSON Exception Occurred", e);
                e.printStackTrace();
            }
        }
    }
            ...
            for (int i = 0; i < quorumPeersObject.length(); i++) {
                ...
                quorumThreads[i] = new Thread(() -> {
                    ...
                    try {
                        ...
                        BlockCommitInitiatorLogger.debug(
                        ...
                        // BlockCommitInitiatorLogger.warn("Alpha quorum (" + quorumID[j] + ") has no
                        // credits");
                        // }
                        // if (cmContent.length() == 0 && role == "alpha") {
                        // BlockCommitInitiatorLogger.warn("Alpha quorum (" + quorumID[j] + ") has no
                        // credits in credit mapping data");
                        // }
                            ...
                            if (!qstContent.toString().contains("empty")) {
                                ...
                                for (int k = 0; k < credObject.length(); k++) {
                                    ...
                                    if (!(Authenticate.verifySignature(hashedCredObject.toString())))
                                        BlockCommitInitiatorLogger.warn("Credit verification failed for Alpha quorum ("
                                                + quorumID[j] + ") credits");

                                    if (cmContent != null) {
                                        for (int l = 0; l < cmContent.length(); l++) {
                                            if ((cmContent.getJSONObject(l).getString("hash") == signHash)) {
                                                BlockCommitInitiatorLogger.warn(
                                                        "Credit verification failed for Alpha quorum (" + quorumID[j]
                                                                + ") credits - Hash matched in Credits Mapping file");
                                            }
                                        }
                                    }
                        ...
                        if (qResponse[j].equals("Auth_Failed")) {
                            ...
                            BlockCommitInitiatorLogger.debug("Signature Received from " + quorumID[j]);
                            if (quorumResponse[index] > minQuorum(quorumSize)) {
                                ...
                                if (Authenticate.verifySignature(detailsToVerify.toString())) {
                                    ...
                                    BlockCommitInitiatorLogger.debug("Quorum Count : " + quorumResponse
                                            + "Signature count : " + quorumSignature.length());
                                } else {
                                    BlockCommitInitiatorLogger.debug("node failed authentication with index " + index
                        ...
                        IPFSNetwork.executeIPFSCommands("ipfs p2p close -t /p2p/" + quorumID[j]);
                        BlockCommitInitiatorLogger.error("IOException Occurred");
                        e.printStackTrace();
                    }
                });
