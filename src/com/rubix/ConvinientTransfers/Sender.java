...
public class Sender {
	...
	public static String senderPayloadSign = null;
	 ...
	 */
	public static JSONObject SendPartA(String data, IPFS ipfs, int port)
			throws JSONException, IOException, Exception, UnknownHostException {
		repo(ipfs);
		resetVariables();
...
//		SenderLogger.debug("Int Part: " + intPart); // 2
		...
		for (int i = 0; i < wholeTokens.length(); i++) {
			...
			if (!(token.exists() && tokenchain.exists())) {
				SenderLogger.info("Token files not available");
				senderMutex = false;
				APIResponse.put("did", senderDidIpfsHash);
				APIResponse.put("tid", "null");
				APIResponse.put("status", "Failed");
...
//			boolean quorumSanityCheck = sanityCheck("Quorum", quorumPeerID, ipfs, port + 10);
			boolean quorumSanityCheck = true;
...
//		SenderLogger.debug("Nodes that pledged tokens: " + Initiator.pledgedNodes);
		...
		if (privateShareFile.exists()) {
			senderPayloadSign = getSignFromShares(pvt, senderPayloadHash);
			payloadSigned.put("authSenderByRecHash", getSignFromShares(pvt, authSenderByRecHash));
			payloadSigned.put("senderPayloadSign", senderPayloadSign);
			for (int i = 0; i < intPart; i++) {
	...
	public static JSONObject SendPartB(JSONObject signPayload, IPFS ipfs, int port)
		...
		forward(receiverPeerId.concat("step"), port, receiverPeerId);
...
//		SenderLogger.debug("********Double Spend Hash*********:  " + doubleSpend);
		...
		if (pinAuth == null) {
			...
			partCPayload.put("senderPayloadSign", senderPayloadSign);
			partCPayload.put("authSenderByRecHash", getSignFromShares(pvt, authSenderByRecHash));
//			SenderLogger.debug(partCPayload.toString());
			// resetVariables();
	...
	public static JSONObject SendPartC(JSONObject signPayload, IPFS ipfs)
			throws JSONException, IOException, InterruptedException, ParseException, Exception {
		PropertyConfigurator.configure(LOGGER_PATH + "log4jWallet.properties");
		SenderLogger.debug("Transfer Part C");
//		SenderLogger.debug(signPayload.toString());
		...
		senderMutex = true;
		long startTime = System.currentTimeMillis();
		if (Initiator.pledge(quorumWithSignsArray, requestedAmount, port)) {
			SenderLogger.debug("Pledging completed");
			...
			SenderLogger.debug("Sender IPFS forwarding to receiver: " + receiverDidIpfsHash + " PeerID: "
					+ receiverPeerId + " - Part C");
			forward(receiverPeerId.concat("step"), port, receiverPeerId);
			SenderLogger.debug("Forwarded to " + receiverPeerId + " on " + port);
			senderSocket = new Socket("127.0.0.1", port);
			...
			senderDetails2Receiver.put("pvtShareBits", signPayload.getString("authSenderByRecHash"));
			...
			try {
				...
				SenderLogger.warn("Receiver " + receiverDidIpfsHash
						+ " could'nt send token chain blocks for hashing and signing");
				executeIPFSCommands(" ipfs p2p close -t /p2p/" + receiverPeerId);
				output.close();
				input.close();
				senderSocket.close();
			...
			if (decimalAmount != 0.0) {
				...
				if (newPart) {
					...
					try {
						...
						SenderLogger.warn("Receiver " + receiverDidIpfsHash
								+ " could'nt send hahsed and signed token chain blocks.");
						executeIPFSCommands(" ipfs p2p close -t /p2p/" + receiverPeerId);
						output.close();
						input.close();
						senderSocket.close();
					...
					writeToFile(TOKENCHAIN_PATH + partTokens.getString(0) + ".json", newPartTokenChain.toString(),
							false);
...
//							SenderLogger.debug("Wholly Spent, Removing token from parts");
							...
							for (int j = 0; j < partContentArray2.length(); j++) {
								...
								writeToFile(PAYMENTS_PATH.concat("PartsToken.json"), partContentArray2.toString(),
										false);
							}
			...
			APIResponse.put("tid", "null");
			APIResponse.put("status", "Failed");
			APIResponse.put("message", "Pledge failed");
			resetVariables();
			return APIResponse;
