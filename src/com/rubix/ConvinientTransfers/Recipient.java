package com.rubix.ConvinientTransfers;
...
public class Recipient {
	public static Logger RecipientLogger = Logger.getLogger(Recipient.class);
	...
	public static String receive() {
		...
		try {
			...
			RECEIVER_PORT = RECEIVER_PORT+39;
			listen(receiverPeerID.concat("step"), RECEIVER_PORT);
			ss = new ServerSocket(RECEIVER_PORT);
			RecipientLogger.debug("Receiver Listening on " + RECEIVER_PORT + " appname " + receiverPeerID);
			...
			try {
				messageRequest = input.readLine();
			} catch (SocketException e) {
				RecipientLogger.warn("Sender Stream Null - Sender Details");
				APIResponse.put("did", "");
			...
			RecipientLogger.debug("Message Request Received: " + messageRequest);

			if (messageRequest == null) {
				RecipientLogger.warn("Sender Stream Null - Message Request");
				APIResponse.put("did", "");
				APIResponse.put("tid", "null");
				APIResponse.put("status", "Failed");
				APIResponse.put("message", "Sender Stream Null - Message Request");
				...
				ss.close();
				return APIResponse.toString();
			} else if (messageRequest.contains("PartB")) {
				try {
				...
				RecipientLogger.debug("Sender Peer ID Received: " + senderPeerID);
				swarmConnectP2P(senderPeerID, ipfs);

				String senderDidIpfsHash = getValues(DATA_PATH + "DataTable.json", "didHash", "peerid", senderPeerID);
				String senderWidIpfsHash = getValues(DATA_PATH + "DataTable.json", "walletHash", "peerid",
						senderPeerID);
				...
				if (!(senderDidIpfsHash.contains("Qm") && senderWidIpfsHash.contains("Qm"))) {
					...
					RecipientLogger.info("Sender details not available in datatable");
					output.close();
					input.close();
					sk.close();
					ss.close();
				...
				if (!senderDIDFile.exists()) {
					...
					RecipientLogger.info("Sender details not available");
					output.close();
					input.close();
					sk.close();
					ss.close();
				...
				RecipientLogger.debug("Sender details authenticated");
				output.println("200");
				...
				try {
					...
					RecipientLogger.warn("Sender Stream Null - Token Details");
					APIResponse.put("did", "");
					APIResponse.put("tid", "null");
					APIResponse.put("status", "Failed");
				...
				RecipientLogger.debug("Token details Received: " + tokenDetails);
				...
				if (TokenDetails.optJSONArray("proof") != null) {
					...
					for (int i = 0; i < proofArray.length(); i++) {
						...
						RecipientLogger.debug(proofFile.getAbsolutePath());
					}
				}
...
//				 JSONArray positionsArray = tokenObject.getJSONArray("positions");
				...
				// TokenReceiverLogger.debug("previousSendersArray is " +
				// previousSendersArray.toString());
				// TokenReceiverLogger.debug("tokenObject is " + tokenObject.toString());
				// TokenReceiverLogger.debug("tokenDetails(base for tokenObj) is " +
...
//					try {
//						RecipientLogger
//								.debug("Checking owners for " + wholeTokens.getString(i) + " Please wait...");
...
//							}
//							RecipientLogger.debug("Previous Owners: " + previousSender.toString());
//							RecipientLogger.debug("Pin owners " + pinOwnersArray.toString());
//
...
//					} catch (IOException e) {
//
//						RecipientLogger.debug("Ipfs dht find did not execute");
//					}
//				}
//				if (multiplePin) {
...
//				RecipientLogger.debug("consensusID is " + consensusID);
				...
				if (!IPFSNetwork.dhtEmpty(consensusIDIPFSHash, ipfs)) {
					RecipientLogger.debug("consensus ID not unique" + consensusIDIPFSHash);
					output.println("421");
					APIResponse.put("did", senderDidIpfsHash);
					APIResponse.put("tid", "null");
					APIResponse.put("status", "Failed");
				...
				if (intPart > 0) {
					...
					// TokenReceiverLogger.debug("Whole Token Chains: " +
					// wholeTokenChainContent.get(0));
					// TokenReceiverLogger.debug("Whole Token Chains: " + new
					// JSONArray(wholeTokenChainContent.get(0)));
...
//							RecipientLogger.debug("Token " + wholeTokens.getString(i) + " is a new token (SHAG structure)");
							if (!tokenChain.getJSONObject(tokenChain.length() - 1).getString("pledgeToken")
									.equals("")) {
//								RecipientLogger.debug("Token " + wholeTokens.getString(i) + " is a pledged token");
								forkResolution = ForkResolution.verifyUnpledgedToken(wholeTokens.getString(i),
										tokenChain.getJSONObject(tokenChain.length() - 1)
												.getJSONArray("tokensPledgedFor").get(0).toString(),
										senderPeerID, receiverPeerID);

							} else {
//								RecipientLogger.debug("Token " + wholeTokens.getString(i)+ " is a new token (SHAG structure) and not pledged");
								forkResolution = ForkResolution.check(forkObject);
							}
						} else {
//							RecipientLogger.debug("Token " + wholeTokens.getString(i) + " is an Eismeer Token");
							forkResolution = ForkResolution.check(forkObject);
						}
						if (!forkResolution) {
							RecipientLogger.debug("Fork found and not resolved for token: " + wholeTokens.getString(i));
							RecipientLogger.debug("Message: " + ForkResolution.resolutionMessage);
				...
				if (wholeTokens.length() > 0) {
					...
					if (!pledged) {
						...
						RecipientLogger.info(pledgeErrorMessage);
						IPFSNetwork.executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
						output.close();
						input.close();
						sk.close();
					...
					if (!(ipfsGetFlag == intPart)) {
						...
						RecipientLogger.info("Tokens not verified");
						IPFSNetwork.executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
						output.close();
						input.close();
						sk.close();
					...
					if (tokenDetailMap.isEmpty()) {
						RecipientLogger.debug("Invalid Content Found in Token");
						...
						RecipientLogger.debug(errorMessage);
						...
						for (String tokenContent : tokenDetailMap.keySet()) {
							...
							if (tokenDetailMap.get(tokenContent) != null
								...
								RecipientLogger.debug(errorMessage1);
								executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
								output.close();
								input.close();
								sk.close();
				...
				RecipientLogger.debug("VERIFYING TOKEN CHAIN(S).. ");

				// For WholeTokenChains
				int chainAutheticityFlag = 0;
				boolean flag = false;
//				RecipientLogger.debug("Verifying whole token chains.. ");
				for (int i = 0; i < intPart; i++) {

					// TokenReceiverLogger.debug("Verifying whole Token Chain : " +
					// wholeTokenChains.getString(i));
...
//						RecipientLogger.debug("Hash to check(hash)  : " + hash);
...
//						RecipientLogger.debug("@@@@@@ hashtocheck is " + hashToCheck);

//						RecipientLogger.debug("@@@@@@ Calculated Hash(hashtocheck) : " + hashToCheck);
...
//							RecipientLogger.debug("Hash values match.");
							...
							String prevSenderPubKeyStr = IPFSNetwork.get(prevSenderPublicKeyIpfsHash, ipfs); // get
																												// sender's
																												// public
																												// key
																												// from
				...
				if (chainAutheticityFlag == 1) {
					...
					RecipientLogger.debug("Token Transfer continuing without TokenChain authentication check. (Bootstrap purpose)");
				...
				if (chainAutheticityFlag == 2) {
					...
					RecipientLogger.info("Token Chain could not be verified.");
				...
				if (chainAutheticityFlag == 3) {
					...
					RecipientLogger.info("Token Chain could not be verified. Malicious activity by sender");
					// TokenReceiverLogger.info("Chain auth flag : "+chainAutheticityFlag);
				...
				if (chainAutheticityFlag == 4) {
					...
					RecipientLogger.info("Token Chain could not be verified. Previous sender's signature not verified");
					// TokenReceiverLogger.info("Chain auth flag : "+chainAutheticityFlag);
				...
				if (flag) {
					RecipientLogger.debug("Whole Token Chains successfully verified !");
				}

				// For PartTokenChains
				int chainAutheticityFlag2 = 0;
				boolean flag2 = false;
				RecipientLogger.debug("Verifying part token chains.. ");
...
//					RecipientLogger.debug("Verifying part Token Chain : " + partTokens.getString(i));
...
//								RecipientLogger.debug("Hash to check  : " + hash);
								...
								// TokenReceiverLogger.debug("Token Chain to be hashed : " +
								// partTokenChainToVerify);
...
//									RecipientLogger.debug("Hash values match.");
							...
							if (lastValueOnChain2.has("hash") && lastValueOnChain2.has("pvtShareBits")) {
								...
								String hashToCheck = calculateHash(partTokenChainToVerify.toString(), "SHA3-256");
//								RecipientLogger.debug("Calculated Hash : " + hashToCheck);
						...
						String hashToCheck = calculateHash(partTokenChainToVerify.toString(), "SHA3-256");
						// TokenReceiverLogger.debug("hashToCheck(partTokenChainToVerify.toString()) is
						// " + hashToCheck);
...
//							RecipientLogger.debug("Hash values match.");
							...
							if (!flag) {
								...
								RecipientLogger.debug("Previous sender's sign also authenticated.");
								// TokenReceiverLogger.debug("Token chain " + wholeTokenChains.getString(i) +
								// "verified");
							}
				...
				if (chainAutheticityFlag2 == 2) {
					...
					RecipientLogger.info("Part Token Chain could not be verified");
				...
				if (chainAutheticityFlag2 == 3) {
					...
					RecipientLogger.info("Part Token Chain could not be verified. Malicious activity by sender");
				...
				if (flag2) {
					RecipientLogger.debug("Part Token Chains verified !");
				}
...
//				boolean ownerCheck = true;
				...
				for (int count = 0; count < wholeTokens.length(); count++) {
					String tokens = null;
					// TokenReceiverLogger.debug("Json array tokenChain value is " +
					// wholeTokens.get(count).toString());
...
//							if (ownerCheck && (tokenChain.length() < minumumStakeHeight)) {
						if (tokenChain.length() < minumumStakeHeight) {
							...
							if (tokenChain.length() > 0) {
								...
								/*
								 * int randomNumber = new Random().nextInt(15); JSONObject
								 * genesisSignaturesContent = genesiObject
								 * .getJSONObject(MiningConstants.QUORUM_SIGN_CONTENT); Iterator randomKey =
								 * genesisSignaturesContent.keys(); for (int i = 0; i < randomNumber; i++) {
								 * randomKey.next(); }
								 */
								/**
								...
								if (stakeDataArray.length() > 0 && stakeDataArray.length() <= 3) {
									...
									for (int i = 0; i < stakeDataArray.length(); i++) {
										...
										stakedTokenSignTC[i] = oneOfThreeStake
									...
									RecipientLogger.debug("mineIDTC length is " + mineIDTC.length);
									...
									for (int stakeCount = 0; stakeCount < mineIDTC.length; stakeCount++) {
										...
										RecipientLogger.debug(mineIDContentJSON.toString());

										JSONObject stakeData = mineIDContentJSON
												.getJSONObject(MiningConstants.STAKE_DATA);

										String stakerDIDMineData = stakeData
												.getString(MiningConstants.STAKED_QUORUM_DID);
										String stakedTokenMineData = stakeData.getString(MiningConstants.STAKED_TOKEN);
										String stakedTokenSignMineData = stakeData
												.getString(MiningConstants.STAKED_TOKEN_SIGN);

										RecipientLogger.debug(stakerDIDTC[stakeCount]);
										RecipientLogger.debug(stakedTokenTC[stakeCount]);
										RecipientLogger.debug(stakedTokenSignTC[stakeCount]);

										RecipientLogger.debug(stakerDIDMineData);
										RecipientLogger.debug(stakedTokenMineData);
										RecipientLogger.debug(stakedTokenSignMineData);
										...
										if (stakerDIDTC[stakeCount].equals(stakerDIDMineData)
											...
											RecipientLogger.debug("array n non array data are same");
											...
											RecipientLogger.debug("detailsToVerify - " + detailsToVerify.toString());
											...
											if (Authenticate.verifySignature(detailsToVerify.toString())) {
												...
												RecipientLogger.debug("dht owner are " + ownersArray.toString()
												...
												if (!minedTokenStatus) {
													RecipientLogger.debug("Staked token is not found with staker DID: "
...
//														ownerCheck = false;
											...
											RecipientLogger.debug("MineID Verification Successful with Staking node: "
													+ stakerDIDTC[stakeCount]);
										} else {
											RecipientLogger.debug("Staking check (2) failed");
//													ownerCheck = false;
											invalidTokens.put(tokens);
										}

										RecipientLogger.debug("Staking check (2) successful for count " + stakeCount);
										// } else {
...
//										ownerCheck = false;
									RecipientLogger.debug("Staked Token is not available!");
						...
						if (!minedTokenStatus) {
							RecipientLogger.debug("Staking check failed: Found staked token but token height < 46");
//								ownerCheck = false;
							invalidTokens.put(tokens);
						}
					...
					for (int k = 0; k < tokenChainContent.length(); k++) {
						if (tokenChainContent.getJSONObject(k).has("role")) {
							if (tokenChainContent.getJSONObject(k).getString("role").equals("Sender")
									&& tokenChainContent.getJSONObject(k).getString("sender")
											.equals(senderDidIpfsHash)) {
								senderCount += tokenChainContent.getJSONObject(k).getDouble("amount");
									...
									&& tokenChainContent.getJSONObject(k).getString("receiver")
											.equals(senderDidIpfsHash)) {
...
//						RecipientLogger.debug("Parts: " + availableParts);
				...
				if (pinCount == intPart) {
					RecipientLogger.debug("Pinned All Tokens");
					...
					RecipientLogger.info(" Transaction failed - Pin Count");
					executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
					output.close();
					input.close();
					sk.close();
			...
			} else if (messageRequest.contains("PartC")) {
				...
				String senderDidIpfsHash = SenderDetails.getString("senderDidIpfsHash");
				int intPart = SenderDetails.getInt("intPart");
				JSONArray wholeTokenChains = SenderDetails.getJSONArray("wholeTokenChains");
				JSONArray wholeTokens = SenderDetails.getJSONArray("wholeTokens");
				JSONArray partTokens = SenderDetails.getJSONArray("partTokens");
				JSONArray partTokenChainsHash = SenderDetails.getJSONArray("partTokenChainsHash");
				JSONObject distributedObject = SenderDetails.getJSONObject("distributedObject");
				JSONObject amountLedger = SenderDetails.getJSONObject("amountLedger");
				double amount = SenderDetails.getDouble("amount");
				double decimalPart = SenderDetails.getDouble("decimalPart");
				JSONObject partTokenChains = SenderDetails.getJSONObject("part-tokenChains");
...
//				RecipientLogger.debug("hash is " + hash);

//				RecipientLogger.debug("Verifying Quorum ...  ");
//				RecipientLogger.debug("Please wait, this might take a few seconds");
...
//						RecipientLogger.debug("verifyQuorumHash is " + verifyQuorumHash);

						String quorumHash = calculateHash(verifyQuorumHash.concat(receiverDidIpfsHash), "SHA3-256");
//						RecipientLogger.debug("quorumHash is " + quorumHash);
//						RecipientLogger.debug("quorumHash is (verifyQuorumHash)concat(receiverDidIpfsHash)");
						...
						for (int i = 0; i < quorumSignatures.length(); i++) {
							...
							if (QuorumPublicKeyIpfsHash.isEmpty()) {
								...
								APIResponse.put("status", "Failed");
								APIResponse.put("message",
										"Quorum data not available for " + quorumMember.getString("quorum_did"));
								RecipientLogger.info("Quorum data not available for "
										+ quorumMember.getString("quorum_did") + ". Please initate sync command");
								IPFSNetwork.executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
								output.close();
								input.close();
								sk.close();
							...
							String quorumPubKeyStr = IPFSNetwork.get(QuorumPublicKeyIpfsHash, ipfs); // get quorum
																										// member's
																										// public key
																										// from
																										// ipfs.
						...
						RecipientLogger.debug("Verified Quorum Count " + quorumSignVerifyCount);
						yesQuorum = quorumSignVerifyCount >= quorumSignatures.length();
					}

//					RecipientLogger.debug("Hash to verify Sender: " + hash);
					JSONObject detailsForVerify = new JSONObject();
					...
					String senderPublicKeyIpfsHash = getPubKeyIpfsHash_DIDserver(senderDidIpfsHash, 1); // get public
																										// key
																										// ipfs hash of
																										// the
																										// sender.
					...
					if (senderPublicKeyIpfsHash.isEmpty()) {
						...
						RecipientLogger.info("Sender's public key data not available for " + senderDidIpfsHash
								+ ". Please initate sync command");
						IPFSNetwork.executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
						output.close();
						input.close();
						sk.close();
					...
					String senderPubKeyStr = IPFSNetwork.get(senderPublicKeyIpfsHash, ipfs); // get sender's public key
																								// from
																								// ipfs.
...
//					RecipientLogger.debug("Quorum Auth : " + yesQuorum + " Sender Auth : " + yesSender);
					if (!(yesSender && yesQuorum)) {
						...
						RecipientLogger.info("Sender / Quorum not verified");
						IPFSNetwork.executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
						output.close();
						input.close();
						sk.close();
					...
					repo(ipfs);
					RecipientLogger.debug("Sender and Quorum Verified");
					output.println("200");
					...
					if (request.equals("Request for new blocks being added to the Token Chains")) {
						...
						RecipientLogger.debug("Request for new blocks being added to the Token Chains");

						// TODO
//						RecipientLogger.debug("intPart: " + intPart);
						for (int i = 0; i < intPart; i++) {
					...
					try {
						...
						if (hashAndSignsforTokenChains.contains("Token chains Not Matching")) {
							RecipientLogger.warn("Token chains Not Matching");
							APIResponse.put("did", "");
							APIResponse.put("tid", "null");
							APIResponse.put("status", "Failed");
						...
						RecipientLogger.warn("Sender Stream Null - Token Chain Updation status");
						APIResponse.put("did", "");
						APIResponse.put("tid", "null");
						APIResponse.put("status", "Failed");
					...
					for (int i = 0; i < partTokens.length(); i++) {
						...
						if (chainFile.exists()) {
							...
							chainArray.put(
									partTokenChainContentArray.getJSONObject(partTokenChainContentArray.length() - 1));
					...
					try {
						...
						RecipientLogger.warn("Sender Stream Null - Pinning Status");
						APIResponse.put("did", "");
						APIResponse.put("tid", "null");
						APIResponse.put("status", "Failed");
					...
					if (pinDetails.equals("Unpinned")) {
						...
						try {
							...
							RecipientLogger.warn("Sender Stream Null - EShare Details");
							APIResponse.put("did", "");
							APIResponse.put("tid", "null");
							APIResponse.put("status", "Failed");
...
//						RecipientLogger.debug("!@#$% 1");
						...
						RecipientLogger.info("Transaction ID: " + tid + "Transaction Successful");
						APIResponse.put("did", senderDidIpfsHash);
						APIResponse.put("tid", tid);
						...
						RecipientLogger.info(" Transaction Successful");
						executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
						output.close();
						input.close();
						sk.close();
					...
					RecipientLogger.info(" Transaction failed - Failed to unpin");
					executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
					output.close();
					input.close();
					sk.close();
				...
				RecipientLogger.info(" Transaction failed - Sender side consensus failed");
				executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
				output.close();
				input.close();
				sk.close();
			...
			APIResponse.put("tid", "null");
			APIResponse.put("status", "Failed");
			APIResponse.put("message", "Unidentified message request");
			RecipientLogger.info(" Transaction failed - unidentified message request");
			executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
			output.close();
			input.close();
			sk.close();
			...
			executeIPFSCommands(" ipfs p2p close -t /p2p/" + senderPeerID);
			RecipientLogger.error("Exception Occurred", e);
			return APIResponse.toString();
		} finally {
			try {
				ss.close();
